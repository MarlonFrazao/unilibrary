...
public class UsuarioDAO extends DAO {
	...
	public List<Usuario> listar() {
		...
		try {
			...
			while (rs.next()) {
				Usuario u = new Usuario(rs.getInt("U.id"), rs.getInt("U.usuario"), rs.getInt("U.senha"),
						new Permissao(rs.getInt("P.id"), rs.getString("P.descricao")), rs.getBoolean("U.status"),
						new Aluno(rs.getInt("A.id"), rs.getString("A.nome"), rs.getString("A.email"),
								new Curso(rs.getInt("C.id"), rs.getString("C.descricao"), rs.getBoolean("C.status")),
								rs.getBoolean("A.status"), rs.getInt("A.matricula")));
				lista.add(u);
			}
	...
	public Usuario obterPorId(int id) {
		...
		try {
			...
			while (rs.next()) {
				u = new Usuario(rs.getInt("U.id"), rs.getInt("U.usuario"), rs.getInt("U.senha"),
						new Permissao(rs.getInt("P.id"), rs.getString("P.descricao")), rs.getBoolean("U.status"),
						new Aluno(rs.getInt("A.id"), rs.getString("A.nome"), rs.getString("A.email"),
								new Curso(rs.getInt("C.id"), rs.getString("C.descricao"), rs.getBoolean("C.status")),
								rs.getBoolean("A.status"), rs.getInt("A.matricula")));
